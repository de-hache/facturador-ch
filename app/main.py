...
def head_html(titulo: str = APP_NAME) -> str:
    return f"""
    <head>
        <meta charset="UTF-8">
        <meta
...
def guardar_ultimo_proceso(texto: str) -> None:
...
def listar_csv_ventas() -> list[str]:
    ...
    for archivo in CARPETA_VENTAS.glob("*.csv"):
        ruta_normalizada = str(archivo).replace("\\", "/")
        archivos.append(ruta_normalizada)
...
def es_error_temporal(error: Exception) -> bool:
...
def crear_voucher_con_reintentos(
    ...
    for intento in range(intentos):
        ...
        try:
            ...
            resultado = afip.ElectronicBilling.createVoucher(
                data_afip
            )
            ...
            registrar_evento(
                "INFO",
                (
                    f"CAE obtenido en intento {numero_intento} - "
                    f"Orden {id_orden}"
                ),
            )
            ...
            ultimo_error = error
            quedan_intentos = intento < intentos - 1

            if es_error_temporal(error) and quedan_intentos:
                indice_espera = min(
                    intento,
                    len(ARCA_ESPERAS) - 1,
                )

                segundos = ARCA_ESPERAS[indice_espera]

                registrar_evento(
                    "ERROR",
                    (
                        f"Error temporal de ARCA en orden {id_orden}. "
            ...
            registrar_evento(
                "ERROR",
                (
                    f"No se pudo obtener CAE para orden {id_orden}. "
                    f"Error final: {error}"
                ),
            )
...
def dashboard():
    ...
    for factura in facturas:
        fecha = escape(str(factura[0]))
        orden = escape(str(factura[1]))
        cliente = escape(str(factura[2]))
        numero_factura = factura[3]
        total = escape(str(factura[4]))
        cae = escape(str(factura[5]))
        estado_factura = escape(str(factura[6]))
    ...
    for evento in eventos:
        fecha = escape(str(evento[0]))
        tipo = escape(str(evento[1]))
        mensaje = escape(str(evento[2]))
...
def procesar(
    ...
    try:
        ruta = Path(archivo_csv).resolve()
        carpeta_ventas_resuelta = CARPETA_VENTAS.resolve()
        ...
        registrar_evento(
            "INFO",
            (
                "Procesamiento iniciado desde Dashboard: "
                f"{archivo_csv}"
            ),
        )
...
def recibir_venta(
    ...
    try:
        ...
        factura_existente = existe_orden_facturada(
            id_orden
        )
...
def reprocesar_pendiente(
    ...
    error_guardado = obtener_error_facturacion(
        id_error
    )
    ...
    registrar_evento(
        "INFO",
        (
            f"Reprocesando pendiente ID {id_error} - "
            f"Orden {orden}"
        ),
    )
...
def consultar_factura(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=(
                "Error consultando factura en ARCA: "
                f"{error}"
            ),
        ) from error
...
def estado():
    return {
        ...
        "punto_venta": PUNTO_VENTA,
        "tipo_comprobante": TIPO_COMPROBANTE,
        "carpeta_ventas": str(CARPETA_VENTAS),
